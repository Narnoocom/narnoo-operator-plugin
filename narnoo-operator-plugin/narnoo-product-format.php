...
/**
*
* @date_created: 29.11.2017
* @description: Gets Parent Post Data
* @return: (array) 
*
*/
function get_narnoo_product_parents(){
	$args = array('post_type' => 'narnoo_product', 'post_parent' => 0, 'posts_per_page'=>'-1');
    // The Query
...
*
* @date_created: 29.11.2017
* @description: Gets Child Post data
* @return: (array) 
*
*/
function get_narnoo_product_children($post_id, $markup = NULL){
    ...
    foreach($children as $child){
    	...
    	if(empty($markup)){
	    	$data[$i]['id'] 		= $child->ID;
	    	$data[$i]['title'] 		= $child->post_title;
	    	$data[$i]['excerpt'] 	= $child->post_excerpt;
	    	$data[$i]['link'] 		= get_the_permalink( $child->ID );
	    	$data[$i]['min_price']  = get_post_meta( $child->ID, 'product_min_price',       true );
	    	$data[$i]['image']  	= get_the_post_thumbnail_url( $child->ID, array(150,150) );
	    	$i++;
    	}else{

    		//find and replace the title.
				$_html = str_replace('{{title}}', 	  	 $child->post_title,     $markup);
				//find and replace the link.
				$_html = str_replace('{{permaLink}}', 	 get_the_permalink( $child->ID ), $_html);
				//find and replace the featureImage.
				$_html = str_replace('{{featureImage}}', get_the_post_thumbnail_url( $child->ID, array(150,150) ), $_html);
				...
				*
				*/
				$minPrice  = get_post_meta($child->ID, 'product_min_price', true); //done
				if(!empty($minPrice)){
...
*
*/
function get_narnoo_related_products( $number=10, $markup = NULL, $post_id = NULL, $children = NULL ){
	...
	/**
	*
	*	If the user has set up parent posts for products then we 
	*	need to exclude these from search results.
	*
	*/
	if( !empty($children) ){
	...
	if ( $the_query->have_posts() ) {
		...
		if( !empty($markup) ){
			...
			while ( $the_query->have_posts() ) {
				...
				$_html = str_replace('{{permaLink}}', 	 get_the_permalink($the_query->post->ID), $_html);
				//find and replace the featureImage.
				$_html = str_replace('{{featureImage}}', get_the_post_thumbnail_url($the_query->post->ID, array(150,150) ), $_html);
	...
	if( !empty($gallery) ){
		...
		if( !empty($markup) ){
			...
			if( !empty($markup['image_class']) ){
				...
				$_imageStyle = 'class="'.$markup['image_class'].'" ';
			}
