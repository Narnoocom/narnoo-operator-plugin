	...
	function get_current_page_data() {
		...
		if ( ! is_null( $list ) ) {
			...
			foreach ( $list->operator_images as $image ) {
				...
				$item['xlarge_image'] 			= $image->xlarge_image_path;
				$item['xxlarge_image'] 			= $image->xxlarge_image_path;
				$item['crop_image'] 			= $image->crop_image_path;
				$item['xcrop_image'] 			= $image->xcrop_image_path;
				$item['image_200'] 				= $image->image_200_path;
				$item['image_400'] 				= $image->image_400_path;
				$item['image_800'] 				= $image->image_800_path;
				$data['items'][] = $item;
			}
		}
